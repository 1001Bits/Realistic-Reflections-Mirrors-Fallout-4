...
namespace REL
{
	...
	public:
		...
		[[nodiscard]] std::size_t             offset() const { return IDDB::get().id2offset_portable(_id); }
	...
	public:
		...
		{
			auto thisID = id();
			// Port map first, keyed by the 1.10.163 id: on a ported runtime the shipped next-gen ids are not
			// trustworthy (1.11.240: id 72684 lands on a dead BeginTechnique copy), the name-joined map is.
			if (_f4ID) {
				if (const auto mapped = DynRefPortIdFallback(_f4ID); mapped != 0)
					return mapped;
			}
