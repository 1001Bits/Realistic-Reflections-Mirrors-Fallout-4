...
namespace RE
{
	...
	public:
		...
		// FO4 PORT: Type enum — values match FO4's actual runtime BSShader::shaderType.
		// VERIFIED via BeginTechnique logging (Feb 2026). Skyrim's enum values are DIFFERENT.
		// Uses plain enum so values can be used as array indices.
		enum Type : int32_t
		{
			None = -1,            // Sentinel / uninitialized (not a real shader type)
			Effect = 0,           // FO4 runtime CONFIRMED (fxp=Effect)
			Utility = 1,          // FO4 runtime CONFIRMED (fxp=Utility)
			BloodSplatter = 2,    // FO4 runtime unconfirmed (placeholder)
			Grass = 3,            // FO4 runtime unconfirmed (placeholder)
			Lighting = 4,         // FO4 runtime CONFIRMED (fxp=DFPrepass/DFLight)
			DistantTree = 5,      // FO4 runtime unconfirmed (placeholder)
			Sky = 7,              // FO4 runtime CONFIRMED (fxp=Sky)
			Particle = 8,         // FO4 runtime unconfirmed (placeholder)
			Water = 10,           // FO4 runtime CONFIRMED (fxp=Water)
			ImageSpace = 12,      // FO4 runtime CONFIRMED (fxp=IS*)
			Total = 20            // Array sizing sentinel
		};
