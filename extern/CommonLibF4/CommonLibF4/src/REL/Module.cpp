...
namespace REL
{
	Module::Module()
	{
		...
		// Fallback: during DLL_PROCESS_ATTACH (static init), GetModuleHandleW
		// may fail because of loader lock. Use GetModuleFileNameW(NULL) to get
		// the current process executable path and extract the filename.
		if (!handle) {
			std::wstring exePath(REX::W32::MAX_PATH, L'\0');
			auto len = REX::W32::GetModuleFileNameW(nullptr, exePath.data(), static_cast<std::uint32_t>(exePath.size()));
			if (len > 0) {
				exePath.resize(len);
				// Extract filename from full path
				auto pos = exePath.find_last_of(L"\\/");
				if (pos != std::wstring::npos) {
					_filename = exePath.substr(pos + 1);
				} else {
					_filename = exePath;
				}
				handle = REX::W32::GetModuleHandleW(_filename.c_str());
...
#if (!defined(ENABLE_FALLOUT_NG) && !defined(ENABLE_FALLOUT_F4))
		// VRONLY build: if we can't get version info (e.g. during early DLL load),
		// assume VR runtime since that's all we support.
		_runtime = Runtime::VR;
		_version = { 1, 2, 72, 0 };
		return;
#endif
